...
private ["_milMarker", "_posMilitary", "_mkr", "_dirmkr", "_prob", "_string", "_iedbool", "_width", "_height", "_storedpos", "_pos", "_text1", "_scudcunt", "_safepos", "_diameter", "_name", "_type", "_enemy", "_positions2", "_listsafeinf", "_listsafe", "_returnpositions", "_debug", "_abort"];
...
_returnpositions = [_mkr, _positions2, 2500, 3000] execVM "scripts\TDMC\AI\TDMC_AI_spawnpositions.sqf";
_time = time + TDMC_MIS_enemycount;

waitUntil {(_time < time) or (!isNil "TDMC_spawnpositions")};

deleteMarkerLocal "TDMC_mkr_AO_1";
...
sleep 1;
["TDMC_tsk_defend", {"Guard Base"}] call A2S_createSimpleTask; 
if (isNil "_posMilitary") then
{ 
	call compile format ["[""TDMC_tsk_defend"", {""Intel reports %5 forming around <marker name=""""%3"""">%4</marker>. Move in and guard the military outpost for 1 hour, in order to complete the task.<br/><br/>%1""}, {""Guard Base""}, {""Guard Base""}] call A2S_setSimpleTaskDescription;", _string, "TDMC_mkr_AO_1", "TDMC_defend_posmkr", "Military Outpost", (TDMC_enemyType call TDMC_fnc_factionToText)];
}else
{
...
TDMC_current_mission = "TDMC_tsk_defend";
TDMC_mission_running = true;
[_enemy, "TDMC_defend_milMarker", _listsafe, _listsafeinf] execVM "scripts\TDMC\missions\defend\defendcheck.sqf";