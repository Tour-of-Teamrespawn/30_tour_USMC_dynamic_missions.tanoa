...
_ammo = switch (toLower _rounds) do
{
	...
	case "82mm laser guided rounds": {["sh_82mm_amos", "82mm laser guided rounds", "8rnd_82mm_mo_shells"]};
	...
	case "155mm laser guided rounds": {["sh_155mm_amos", "155mm laser guided rounds", "32rnd_155mm_mo_shells"]};
	...
	if (isServer) then
	{
		...
		[_interval, _ammo, _position, _amount, _radius] spawn
		{
			...
			{		
				...
				if (_ammo select 1 in ["155mm laser guided rounds", "82mm laser guided rounds"]) then 
				{
					{
						if (!isNull laserTarget _x) then 
						{
							if ((_pos distance (laserTarget _x)) < 50) exitWith {_pos = getPosATL (laserTarget _x)};
						};
